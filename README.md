...
There are additional large files (> 1MB), mainly FITS images, which are needed for some of the sections, these can be downloaded [fix-here](http://math_research.uct.ac.za/~siphelo/admin/interferometry/data/fundamentals_fits.tar.gz) , the original simulated KAT-7 measurement sets can be downloaded [fix-here](math_research.uct.ac.za/~siphelo/admin/interferometry/data/simulated_KAT-7_ms.tar.gz). These are tarballs which should be extracted in the data directory.
